                    ...
                    if ($dubl_row[0]->c_title != $qcat->quiz_title || $dubl_row[0]->c_created_time != $qcat->quiz_createtime) {
                        ...
                        $query .= "VALUES(" .
                            $db->quote($free_id) . "," . $db->quote($categories_relations_quiz[$qcat->quiz_category]) . "," . $db->quote($qcat->quiz_number_times) . ",
								...
								" . $db->quote($qcat->quiz_unpass_message) . ", " . $db->quote(@$qcat->quiz_enable_review) . ", " . $db->quote($qcat->quiz_email_to) . "," . $db->quote($qcat->quiz_email_to_introduction) . "," . $db->quote($qcat->quiz_email_to_ending) . ",
                        ...
                        $asset->store();
                        if (!$user->authorise('core.view', $asset_name) && $user->authorise('core.view', $asset_name) != $qcat->quiz_guest
                        ) {
