...
class JoomlaquizModelResults extends JModelList
{
            ...
            if (!empty($search)) {
                if (stripos($search, 'id:') === 0) {
                    ...
                    if (stripos(trim($search), 'code:') === 0) {
                        $query->where('CONCAT("' . JText::_('COM_JOOMLAQUIZ_SHORTCODE_ADJUSTER') . '"sq.c_id,sq.c_student_id,sq.c_total_score) = "' . base_convert(str_replace('code:',
                                '', trim($search)), 36, 10) . '"');
                    } else {
    ...
    public function getLists()
    {
        ...
        $quiz = JHTML::_('select.genericlist', $quizzes, 'filter_quiz_id',
            'class="text_area" style="max-width: 300px;" size="1" ' . $javascript, 'value', 'text',
            $app->getUserStateFromRequest('results.filter.quiz_id', 'filter_quiz_id'));
        $lists['quiz'] = $quiz;
        ...
        $lists['passed'] = JHTML::_('select.genericlist', $opt, 'filter_passed',
            'class="text_area" style="max-width: 300px;" size="1" ' . $javascript, 'value', 'text',
            $app->getUserStateFromRequest('results.filter.passed', 'filter_passed'));
        ...
        $lists['user'] = JHTML::_('select.genericlist', $users, 'filter_user_id',
            'class="text_area" style="max-width: 300px;" size="1" ' . $javascript, 'value', 'text',
            $app->getUserStateFromRequest('results.filter.user_id', 'filter_user_id'));
                    ...
                    if ($result->c_quiz_id == $quiz_id) {
                        ...
                        foreach ($questions as $q_number => $question) {
                            if (in_array($question->c_id, $student_questions)) {
                                ...
                                $data['answer'] = $answer;
                                $answer = (method_exists($className,
                                    'onGetAdminCsvData')) ? $appsLib->triggerEvent('onGetAdminCsvData',
                                    $data) : array('');
                                $answer = $data['answer'];
    ...
    public function JQ_csv_report($cid)
    {
        ...
        for ($i = 0, $n = count($csv_rows); $i < $n; $i++) {
            if ($csv_rows[$i]->c_pool) {
                ...
                if (!empty($qch_ids_type_10)) {
                    $query = "SELECT SUM(a_point) FROM #__quiz_t_choice WHERE c_question_id IN (" . implode(',',
                            $qch_ids_type_10) . ") AND c_right = 0";
                    $database->SetQuery($query);
                    $total_score += $database->LoadResult();
                }
        ...
        for ($i = 0, $n = count($csv_rows); $i < $n; $i++) {
            ...
            $str .= ($csv_rows[$i]->username != '' ? $csv_rows[$i]->username : (($csv_rows[$i]->user_name != '') ? $csv_rows[$i]->user_name : (($csv_rows[$i]->user_email != '') ? $csv_rows[$i]->user_email : JText::_('COM_JOOMLAQUIZ_ANONYMOUS_USER')))) . '","' . $csv_rows[$i]->user_email . '","' . $csv_rows[$i]->c_total_score . '","' . JHtml::_('date',
                    $csv_rows[$i]->c_date_time, 'Y-m-d H:i:s') . '"';
            ...
            $str .= ',"' . str_pad($tot_min, 2, "0", STR_PAD_LEFT) . ":" . str_pad($tot_sec, 2, "0",
                    STR_PAD_LEFT) . '","';
