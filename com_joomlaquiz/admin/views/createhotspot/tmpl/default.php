...
<script language="javascript" type="text/javascript">
    ...
    function jq_SaveHotSpotArea() {
        if ((hs_lefttop_x == 0) && (hs_lefttop_y == 0) && (hs_rightbottom_x == 0) && (hs_rightbottom_y == 0)) {
            alert("<?php echo JText::_('COM_JOOMLAQUIZ_HOTSPOT_AREA_NOT', true);?>");
            return false;
        } else {
...
<table cellpadding="0" cellspacing="0" border="0" width="100%" height="600px">
    <tr>
...
<div id="div_hotspot_rec"
     style="background-color:#FFFFFF; <?php if (preg_match('/MSIE ([0-9].[0-9]{1,2})/', $_SERVER['HTTP_USER_AGENT'])) {
         echo "filter:alpha(opacity=50);";
     } ?> -moz-opacity:.50; opacity:.50; border:1px solid #000000; position:absolute; left:<?php echo $hs_lefttop_x + 10 ?>px; top:<?php echo $hs_lefttop_y + 100 ?>px; width:<?php echo($hs_rightbottom_x - $hs_lefttop_x) ?>px; height:<?php echo($hs_rightbottom_y - $hs_lefttop_y) ?>px; z-index:10; "
     onMouseDown="JQ_img_click_handler(event)" onmousemove="JQ_img_move_handler(event)"><img
            src="<?php echo JURI::root() ?>administrator/components/com_joomlaquiz/assets/images/blank.png" border="0"
            width="1" height="1"></div>
<img id="img_hotspot" src="<?php echo JURI::root(); ?>/images/joomlaquiz/images/<?php echo $image_name ?>"
     onclick="JQ_img_click_handler(event)" onMouseMove="JQ_img_move_handler(event)"
     style="position:absolute; left:10px;top:100px; z-index:1;"/>