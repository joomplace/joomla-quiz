...
<?php echo $this->loadTemplate('menu'); ?>
    <form action="<?php echo JRoute::_('index.php?option=com_joomlaquiz&view=lpaths'); ?>" method="post"
            ...
            <div style="margin-top: 10px;" id="j-main-container">
                ...
                <table class="table table-striped" id="quizcategoriesList">
                    ...
                    <tr>
                        ...
                        <th width="40%">
                            <?php echo JHtml::_('grid.sort', 'COM_JOOMLAQUIZ_TITLE2', 'title', $listDirn, $listOrder); ?>
                        </th>
                        <th width="20%">
                    ...
                    <?php if (sizeof($this->items)) {
                        foreach ($this->items as $i => $item) :
                            ...
                            $canEdit = $user->authorise('core.edit', $extension . '.lpaths.' . $item->id);
                            $canCheckin = $user->authorise('core.admin', 'com_checkin');
                            $canChange = $user->authorise('core.edit.state', $extension . '.lpaths.' . $item->id) && $canCheckin;
                            ?>
                            <tr class="row<?php echo $i % 2; ?>" sortable-group-id="1">
                                ...
                                <td class="has-context">
                                    <div>
                                        <?php if ($canEdit) : ?>
                                            <a href="<?php echo JRoute::_('index.php?option=com_joomlaquiz&task=lpath.edit&id=' . $item->id); ?>">
                                ...
                                <td>
                                    <div>
                                        <?php echo JHtml::_('jgrid.published', $item->published, $i, 'lpaths.', $canChange); ?>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php } else { ?>
                        <tr>
                            <td colspan="5" align="center">
                                <?php echo JText::sprintf('COM_JOOMLAQUIZ_NOLPATHCREATED'); ?>
