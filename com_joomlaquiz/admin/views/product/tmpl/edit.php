...
<form action="<?php echo JRoute::_('index.php?option=com_joomlaquiz&layout=edit&pid=' . (int)$this->item->pid); ?>"
      enctype="multipart/form-data" method="post" name="adminForm" id="product-form" class="form-validate">
    <div id="j-main-container" class="span10 form-horizontal">
        <fieldset class="adminform">
            <div class="control-group form-inline">
                <label class="control-label" for="jform_product_id"
                       id="jform_product_id-lbl"><?php echo JText::_('COM_JOOMLAQUIZ_PRODUCT_NAME2') ?></label>
                <div class="controls">
                    <input type="text" size="30" value="<?php echo @$this->lists['name'] ?>" id="jform_name"
                           name="jform[name]">
                </div>
            </div>
            <?php if (strlen($this->lists['products']) != 0): ?>
                <div class="control-group form-inline">
                    <label class=" control-label" for="jform_product_id"
                           id="jform_product_id-lbl"><?php echo JText::_('COM_JOOMLAQUIZ_OR_SELECT_VM') ?></label>
                    <div class="controls">
                        <?php if (!$this->lists['no_virtuemart']) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo JText::_('COM_JOOMLAQUIZ_OR_SELECT_VM'); ?><?php echo $this->lists['products'];
                        } else { ?><input  type="hidden" name="product_id"
                                           value="<?php echo $this->lists['product_id']; ?>" /><?php } ?>
                    </div>
                </div>
            <?php endif; ?>
            <ul class="nav nav-tabs" id="questionTabs" style="width:110%">
                <li class="active"><a href="#quizzes"
                                      data-toggle="tab"><?php echo JText::_('COM_JOOMLAQUIZ_ADMINISTRATION_QUIZZES_LIST'); ?></a>
                </li>
                <li><a href="#lpaths"
                       data-toggle="tab"><?php echo JText::_('COM_JOOMLAQUIZ_ADMINISTRATION_LPATHS_LIST'); ?></a></li>
            </ul>
                            ...
                            foreach ($this->lists['quiz'] as $quiz) {
                                ...
                                <tr>
                                    <td class="nowrap center">
                                        <input type="checkbox" name="q_ids[]" id="q_ids_<?php echo $quiz->value; ?>"
                                               value="<?php echo $quiz->value; ?>"
                                               onclick="disablerow(this, 'q')" <?php echo($checked ? 'checked ' : ''); ?>/>
                                    </td>
                                    ...
                                    <td class="has-context">
                                        ...
                                        <input type="radio" name="q_access_<?php echo $quiz->value; ?>"
                                               id="q_access_0_<?php echo $quiz->value; ?>" value="0"
                                               onclick="viewaccessarea(this, <?php echo $quiz->value; ?>, 'q');" <?php echo ($checked ? '' : 'disabled ') . (!$period ? 'checked ' : ''); ?> />
                                        ...
                                        <input type="radio" name="q_access_<?php echo $quiz->value; ?>"
                                               id="q_access_1_<?php echo $quiz->value; ?>" value="1"
                                               onclick="viewaccessarea(this, <?php echo $quiz->value; ?>, 'q');" <?php echo ($checked ? '' : 'disabled ') . ($period ? 'checked ' : ''); ?> />
                                        <label for="q_access_1_<?php echo $quiz->value; ?>"><?php echo JText::_('COM_JOOMLAQUIZ_PERIOD'); ?></label>.
                                    </td>
                                    <td class="has-context">
									<span id="q_area_0_<?php echo $quiz->value; ?>"
                                          style="display: <?php echo($period ? 'none' : 'inline'); ?>;">
										<?php echo JText::_('COM_JOOMLAQUIZ_NUMBER_OF_DAYS'); ?>
                                        <input class="input-small" type="text" size="5" maxlength="5"
                                               name="q_xdays_<?php echo $quiz->value; ?>"
                                               id="q_xdays_<?php echo $quiz->value; ?>"
                                               value="<?php echo(@$relations['q'][$quiz->value]['xdays'] ? $relations['q'][$quiz->value]['xdays'] : 0); ?>" <?php echo($checked ? '' : 'disabled '); ?> />
									</span>
                                        <span id="q_area_1_<?php echo $quiz->value; ?>"
                                              style="display: <?php echo($period ? 'inline' : 'none'); ?>;">
                                    ...
                                    <span>
                                        ...
                                        <input class="input-small" type="text" size="5" maxlength="3"
                                               name="q_attempts_<?php echo $quiz->value; ?>"
                                               id="q_attempts_<?php echo $quiz->value; ?>"
                                               value="<?php echo(@$relations['q'][$quiz->value]['attempts'] ? $relations['q'][$quiz->value]['attempts'] : 0); ?>" <?php echo($checked ? '' : 'disabled '); ?> />
								    </span>
                                    </td>
                                </tr>
                                <?php
                    ...
                    <table class="adminform">
                        ...
                        <table class="table table-striped">
                            ...
                            foreach ($this->lists['lpath'] as $lpath) {
                                ...
                                <tr>
                                    <td class="nowrap center">
                                        <input type="checkbox" name="l_ids[]" id="l_ids_<?php echo $lpath->value; ?>"
                                               value="<?php echo $lpath->value; ?>"
                                               onclick="disablerow(this, 'l')" <?php echo($checked ? 'checked ' : ''); ?>>
                                    </td>
                                    <td class="has-context">
                                    ...
                                    <td class="has-context">
                                        <?php echo JText::_('COM_JOOMLAQUIZ_ACCESS_TO_COURSE'); ?>
                                        <input type="radio" name="l_access_<?php echo $lpath->value; ?>"
                                               id="l_access_0_<?php echo $lpath->value; ?>" value="0"
                                               onclick="viewaccessarea(this, <?php echo $lpath->value; ?>, 'l');" <?php echo ($checked ? '' : 'disabled ') . (!$period ? 'checked ' : ''); ?> />
                                        <label for="l_access_0_<?php echo $lpath->value; ?>"><?php echo JText::_('COM_JOOMLAQUIZ_X_DAYS'); ?></label>
                                        <?php echo JText::_('COM_JOOMLAQUIZ_OR2'); ?>
                                        <input type="radio" name="l_access_<?php echo $lpath->value; ?>"
                                               id="l_access_1_<?php echo $lpath->value; ?>" value="1"
                                               onclick="viewaccessarea(this, <?php echo $lpath->value; ?>, 'l');" <?php echo ($checked ? '' : 'disabled ') . ($period ? 'checked ' : ''); ?> />
                                        <label for="l_access_0_<?php echo $lpath->value; ?>"><?php echo JText::_('COM_JOOMLAQUIZ_PERIOD'); ?></label>.
                                    </td>
                                    <td class="has-context">
									<span id="l_area_0_<?php echo $lpath->value; ?>"
                                          style="display: <?php echo($period ? 'none' : 'inline'); ?>;">
										<?php echo JText::_('COM_JOOMLAQUIZ_NUMBER_OF_DAYS'); ?>
                                        <input class="input-small" type="text" size="5" maxlength="5"
                                               name="l_xdays_<?php echo $lpath->value; ?>"
                                               id="l_xdays_<?php echo $lpath->value; ?>"
                                               value="<?php echo(@$relations['l'][$lpath->value]['xdays'] ? $relations['l'][$lpath->value]['xdays'] : 0); ?>" <?php echo($checked ? '' : 'disabled '); ?> />
									</span>
                                        <span id="l_area_1_<?php echo $lpath->value; ?>"
                                              style="display: <?php echo($period ? 'inline' : 'none'); ?>;">
										<?php echo JText::_('COM_JOOMLAQUIZ_PERIOD_FROM'); ?>
                                    ...
                                    <span>
									    <?php echo JText::_('COM_JOOMLAQUIZ_NUMBER_OF_ATTEMPTS'); ?>
                                        <input class="input-small" type="text" size="5" maxlength="3"
                                               name="l_attempts_<?php echo $lpath->value; ?>"
                                               id="l_attempts_<?php echo $lpath->value; ?>"
                                               value="<?php echo(@$relations['l'][$lpath->value]['attempts'] ? $relations['l'][$lpath->value]['attempts'] : 0); ?>" <?php echo($checked ? '' : 'disabled '); ?> />
								    </span>
                                    </td>
                                </tr>
                                <?php
