...
//			$query = "SELECT * FROM #__quiz_t_quiz WHERE c_id = '{$quiz_id}'";
        ...
        if (!$order_id && !$rel_id) {
            if ($quiz->c_number_times) {
                ...
                $query = "SELECT COUNT(*) FROM #__quiz_r_student_quiz WHERE `c_quiz_id` = '".$quiz_id."' ". ($quiz->c_allow_continue ? ' AND c_finished = 1 ': ''). " AND {$unique_pass_str} AND `params` = ".$db->quote($cust_params)." ";
                $db->SetQuery( $query );
                $number_times_passed = (int)$db->loadResult();
            ...
            if ($order_id < 1000000000) {
                if($product_type == 'vm'){
                    ...
                    $query->select($db->qn(qp) . '.*')
                        ...
                        ->where($db->qn('qp.id') . ' = ' . $db->q($rel_id))
                        ->where($db->qn('ebr.id') . ' = ' . $db->q($order_id))
            ...
            $db->SetQuery( $query );
            $product_params_attempts = (int)$db->loadResult();
            ...
            $db->SetQuery( $query );
            $products_stats_attempts = (int)$db->loadResult();
    ...
    public static function JQ_checkPackage($package_id, $rel_id, $product_type) {
        ...
        if ($product_type == 'vm') {//for VirtueMart
            ...
            $query->select($db->qn('qp') . '.*')
                ...
                ->innerJoin($db->qn('#__virtuemart_order_items', 'vm_oi')
                    . ' ON '
                    . $db->qn('vm_oi.virtuemart_order_id') . ' = ' . $db->qn('vm_o.virtuemart_order_id')
                )
                ->innerJoin($db->qn('#__quiz_products', 'qp')
                    . ' ON '
                    . $db->qn('qp.pid') . ' = ' . $db->qn('vm_oi.virtuemart_product_id')
                )
                ->where($db->qn('vm_o.virtuemart_user_id') . ' = ' . $db->q($my->id))
                ->where($db->qn('vm_o.virtuemart_order_id') . ' = ' . $db->q($package_id))
                ->where($db->qn('qp.id') . ' = ' . $db->q($rel_id))
                ->where($db->qn('qp.pid_type') . ' = \'vm\'')
            ...
            $query->select($db->qn('qp') . '.*')
                ...
                ->leftJoin($db->qn('#__j2store_orders', 'jo')
                    . 'ON'
                    . $db->qn('jo.j2store_order_id') . ' = ' . $db->q($package_id)
                )
                ->where($db->qn('qp.id') . ' = ' . $db->q($rel_id))
                ->where($db->qn('jo.user_id') . ' = ' . $db->q($my->id))
                ->where($db->qn('jo.order_state_id') . ' = 1 ');
        } else if($product_type == 'eb'){//For Event Booking
            $query->clear();
            $query->select($db->qn('qp') . '.*')
                ->from($db->qn('#__quiz_products', 'qp'))
                ->leftJoin($db->qn('#__eb_registrants', 'ebr')
                    . 'ON'
                    . $db->qn('ebr.id') . ' = ' . $db->q($package_id)
                )
                ->where($db->qn('qp.id') . ' = ' . $db->q($rel_id))
                ->where($db->qn('ebr.user_id') . ' = ' . $db->q($my->id))
                ->where($db->qn('ebr.published') . ' = 1')
            ;
        } else {//For JoomlaQuiz products
            $query = "SELECT qp.*"
                . "\n FROM `#__quiz_payments` AS p"
                . "\n INNER JOIN `#__quiz_products` AS qp ON qp.pid = p.pid"
        ...
        $query->select($db->qn('qps'). '.*')
            ...
            ->where($db->qn('qps.qp_id') . ' = ' . $db->q($rel_id))
            ->where($db->qn('qps.oid') . ' = ' . $db->q($package_id))
        ;
//			$query = "SELECT *"
...
//			;
        ...
        if($product_data->xdays) {
            if(!empty($products_stat) && array_key_exists($rel_id, $products_stat)) {
                ...
                if ($product_type == 'vm') {//For VirtueMart
                    ...
                    $query->clear();
                    $query->select('UNIX_TIMESTAMP(' . $db->qn('jo.created_on') . ')')
                        ->from($db->qn('#__j2store_orders', 'jo'))
                        ->where($db->qn('jo.j2store_order_id') . ' = ' . $db->q($package_id))
                        ->where($db->qn('jo.order_state_id') . ' = 1');
                } else if($product_type == 'eb'){//For Event Booking
                    $query->clear();
                    $query->select('UNIX_TIMESTAMP(' . $db->qn('ebr.payment_date') . ')')
                        ->from($db->qn('#__eb_registrants', 'ebr'))
        ...
        $query->select ('SUM(c_point)')
            ...
            //->where("`c_id` IN (".$db->qn($qch_ids).")")
            ->where("`c_id` IN (".$qch_ids.")")
            ->where($db->qn('published')." = 1")
            ->where($db->qn('c_type')." <> 11");

        $db->SetQuery( $query );
        $max_score = $db->LoadResult();
        ...
        if(count($folders)){
            foreach($folders as $folder){
                ...
                $db->setQuery("SELECT `enabled` FROM `#__extensions` WHERE folder = 'joomlaquiz' AND type = 'plugin' AND element = '".$folder."'");
                $enabled = $db->loadResult();
