...
<div class="jq_results_container">
    <form name="adminForm" id="adminForm"
          action="index.php?option=com_joomlaquiz&view=results<?php echo JoomlaquizHelper::JQ_GetItemId(); ?>"
          method="post">
        <table class="jq_results_container_table table-striped" cellpadding="10" cellspacing="10" border="0"
               width="100%">
            <tr>
            ...
            foreach ($displayData->items as $i => $row) {
                ...
                <tr class="sectiontableentry<?php echo $k; ?>">
                    ...
                    <td align="left">
                        <?php if ($row->name) { ?>
                            <span title="<?php echo JFactory::getUser($row->c_student_id)->username; ?>"
                                  alt="<?php echo JFactory::getUser($row->c_student_id)->username; ?>"
                                  style="cursor: help;"><?php echo JFactory::getUser($row->c_student_id)->name; ?></span>
                        <?php } else { ?>
                            <span title="<?php echo $row->user_surname; ?>" alt="<?php echo $row->user_surname; ?>"
                                  style="cursor: help;"><?php echo $row->user_name; ?></span>
                        <?php } ?>
                    </td>
                    <td align="left">
                    ...
                    <td align="left">
                        <?php if ($row->c_passed == -1) {
                            echo JText::_('COM_JQ_SCORE_PENDING');
                        } else { ?>
                    ...
                    <td align="center">
                        <?php if ($row->c_passed == -1) { ?><strong>?</strong><?php } else { ?>
                            <img src="<?php echo JURI::root(); ?>components/com_joomlaquiz/assets/images/<?php echo $img_passed; ?>"
                                 border="0" alt="<?php echo $alt_passed; ?>"/>
                        <?php } ?>
                    </td>
                    <td align="left">
                        <?php
                    ...
                    <td align="center">
                        <?php if ($row->c_certificate && $row->c_passed): ?>
                            <a onclick="window.open ('<?php echo JRoute::_("index.php?option=com_joomlaquiz&task=printcert.get_certificate&stu_quiz_id=" . $row->id . ".&user_unique_id=" . $row->unique_id); ?>','blank');"
                               href="javascript:void(0)"><?php echo JText::_('COM_JOOMLAQUIZ_DOWNLOAD'); ?></a>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php
