...
class JoomlaquizModelAjaxaction extends JModelList
{
    ...
    public function JQ_process_ajax($jq_task)
    {
        ...
        switch ($jq_task) {
            case 'start': $jq_ret_str = $this->JQ_StartQuiz();
                break;
            case 'next': $jq_ret_str = $this->JQ_NextQuestion();
                break;
            case 'nextFinish': $jq_ret_str = $this->JQ_NextQuestionFinish();
                break;
            case 'finish_stop': $jq_ret_str = $this->JQ_FinishQuiz();
                break;
            case 'email_results': $jq_ret_str = $this->JQ_emailResults();
                break;
            case 'review_start': $jq_ret_str = $this->JQ_StartReview();
                break;
            case 'review_next': $jq_ret_str = $this->JQ_NextReview();
                break;
            case 'preview_quest': $jq_ret_str = $this->JQ_QuestPreview();
                break;
            case 'next_preview': $jq_ret_str = $this->JQ_NextPreview();
                break;
            case 'goto_quest': $jq_ret_str = $this->JQ_SeekQuestion();
                break;
            case 'showpage': $jq_ret_str = $this->JQ_ShowPage();
                break;
            case 'prev': $jq_ret_str = $this->JQ_PrevQuestion();
                break;
            case 'check_blank': $jq_ret_str = $this->JQ_CheckBlank();
                break;
            case 'ajax_plugin': $jq_ret_str = $this->JQ_ajaxPlugin();
                break;
    ...
    public function JQ_FinishQuiz()
    {
        ...
        $result_mode = 0;
        if (!empty($_SESSION['jq_result_mode']) && is_array($_SESSION['jq_result_mode']) && count($_SESSION['jq_result_mode']) == 3 && $_SESSION['jq_result_mode'][0] == $rel_id && $_SESSION['jq_result_mode'][1] == $quiz_id) {

            $result_mode = 1;
            unset($_SESSION['jq_result_mode']);
        } else if (!empty($_SESSION['jq_result_mode_lid']) && is_array($_SESSION['jq_result_mode_lid']) && count($_SESSION['jq_result_mode_lid']) == 2 && $_SESSION['jq_result_mode_lid'][0] == $lid && $_SESSION['jq_result_mode_lid'][1] == $quiz_id) {

            $result_mode = 2;
            unset($_SESSION['jq_result_mode_lid']);
        } elseif (!empty($_SESSION['jq_result_mode_5']) && is_array($_SESSION['jq_result_mode_5']) && count($_SESSION['jq_result_mode_5']) == 2 && $_SESSION['jq_result_mode_5'][1] == $quiz_id) {
            ...
            if ($stu_quiz_id) {
                ...
                if ($cur_tmpl) {
                    ...
                    if ($c_manual || !$quiz_info->c_show_result) {
                        ...
                        if ($p1 && $p2)
                            $results_txt = JoomlaquizHelper::jq_substr($results_txt, 0, $p1) . JoomlaquizHelper::jq_substr($results_txt, $p2 + 29);
                    }
                    ...
                    } elseif (JoomlaquizHelper::isQuizAttepmts($quiz_id, 0, 0, 0, $tmp) && (!$quiz->one_time || !$db->setQuery($query)->loadResult())) {
                        $is_attempts = true;
    ...
    public function JQ_GetQuestData($q_data, $i_quiz_id, $stu_quiz_id = 0)
    {
        ...
        $query = "SELECT c_id, ordering FROM `#__quiz_t_question` WHERE c_id IN ('"
        ...
        $query = "SELECT * FROM `#__quiz_t_question` WHERE `ordering` >= '{$last_ordering}' AND c_id <> '{$last_pbreak_id}' AND c_id IN ('"
        ...
        foreach ($all_quests as $q_data) {
            ...
            if ($seek_quest_id) {
                if ($q_data->c_id != $seek_quest_id) {
                    ...
                    if ($stu_quiz_id) {
                        $query = "SELECT c_id, c_attempts FROM #__quiz_r_student_question WHERE c_stu_quiz_id = '"
            ...
            if ($cur_template) {
                ...
                $query = "SELECT `c_flag_question` FROM `#__quiz_r_student_question` WHERE `c_stu_quiz_id` = '"
                    . $stu_quiz_id . "' and `c_question_id` = '" . $q_data->c_id
                    . "'";
                $database->SetQuery($query);
                $c_flag_question = ($database->LoadResult()) ? $database->LoadResult() : 0;

                $query = "SELECT c_id FROM #__quiz_r_student_question AS sq WHERE c_stu_quiz_id = '"
                    . $stu_quiz_id . "' AND c_question_id = '" . $q_data->c_id
                    . "'";
                $database->SetQuery($query);
                ...
                $ret_str .= "\t" . '<skip_question>'
                    . $this->JQ_GetNextQuestion($qch_ids, $i_quiz_id, $stu_quiz_id, $q_data->c_id) . '</skip_question>'
                    . "\n";
                ...
                //if randomize quest
                $query = "SELECT c_random from #__quiz_t_question WHERE c_id = '"
        ...
        if (!$seek_quest_id && $stu_quiz_id && is_array($all_quests) && count($all_quests) && is_array($qchids) && count($qchids)
            ...
            $query = "SELECT c_question_id FROM #__quiz_r_student_question WHERE c_stu_quiz_id = '"
                . $stu_quiz_id . "'";
            $database->SetQuery($query);
            $q_ids = $database->loadColumn();
    ...
    public function JQ_TimeIsUp($quiz, $stu_quiz_id = null)
    {
        ...
        if ($cur_tmpl) {
            ...
            for ($q = 0, $qn = count($quest_ids); $q < $qn; $q++) {
                ...
                $this->JQ_SaveAnswer(
                    $stu_quiz_id, $quest_id, $answer, $qtype, $c_penalty, $is_avail, $is_correct, $is_no_attempts, $questtype1_answer_incorrect, $got_one_correct, $c_quest_cur_attempt, $c_all_attempts
                );
            }
        }
