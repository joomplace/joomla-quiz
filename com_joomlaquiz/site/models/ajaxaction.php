...
class JoomlaquizModelAjaxaction extends JModelList
{
    ...
    public function JQ_process_ajax($jq_task)
    {
        ...
        switch ($jq_task) {
            case 'start':
                $jq_ret_str = $this->JQ_StartQuiz();
                break;
            case 'next':
                $jq_ret_str = $this->JQ_NextQuestion();
                break;
            case 'nextFinish':
                $jq_ret_str = $this->JQ_NextQuestionFinish();
                break;
            case 'finish_stop':
                $jq_ret_str = $this->JQ_FinishQuiz();
                break;
            case 'email_results':
                $jq_ret_str = $this->JQ_emailResults();
                break;
            case 'review_start':
                $jq_ret_str = $this->JQ_StartReview();
                break;
            case 'review_next':
                $jq_ret_str = $this->JQ_NextReview();
                break;
            case 'preview_quest':
                $jq_ret_str = $this->JQ_QuestPreview();
                break;
            case 'next_preview':
                $jq_ret_str = $this->JQ_NextPreview();
                break;
            case 'goto_quest':
                $jq_ret_str = $this->JQ_SeekQuestion();
                break;
            case 'showpage':
                $jq_ret_str = $this->JQ_ShowPage();
                break;
            case 'prev':
                $jq_ret_str = $this->JQ_PrevQuestion();
                break;
            case 'check_blank':
                $jq_ret_str = $this->JQ_CheckBlank();
                break;
            case 'ajax_plugin':
                $jq_ret_str = $this->JQ_ajaxPlugin();
                break;
        ...
        $jq_ret_str = preg_replace_callback($regex, function ($matches) {
            return 'href="' . JRoute::_(str_replace('href="', '', $matches[0]));
        }, $jq_ret_str);
    ...
    protected function userHasAccess($quiz, $user = null)
    {
        ...
        if (
            ...
            (!$user->authorise('core.view',
                    'com_joomlaquiz.quiz.' . $quiz->c_id) /* c_guest must be excluded */ && (!$user->id && !$quiz->c_guest))
        ) {
            return false;
        } else {
            return true;
    ...
    public function JQ_StartQuiz()
    {
        ...
        if ($quiz_id) {
            ...
            switch ($quiz->c_pool) {
                case '1':
                    $query = "SELECT q_count FROM #__quiz_pool WHERE q_id = '" . $quiz_id . "' LIMIT 1";
                    $database->SetQuery($query);
            ...
            if ($old_quiz) {
                ...
                $query = "SELECT q.* FROM #__quiz_t_question as q LEFT JOIN `#__quiz_t_qtypes` as `b` ON b.c_id = q.c_type LEFT JOIN `#__extensions` as `e` ON (CONVERT (e.element USING utf8) COLLATE utf8_unicode_ci) = b.c_type WHERE q.c_id IN ('" . implode("','",
                        $qchids) . "') AND q.published = 1 AND e.folder = 'joomlaquiz' AND e.type = 'plugin' AND e.enabled = 1 ORDER BY q.ordering, q.c_id";
                $database->SetQuery($query);
                $q_data = $database->LoadObjectList();
...
if($resultQRQ){
                    ...
                    $this->JQ_SaveAnswer($stu_quiz_id, $quest_id, $answer, $qtype, $c_penalty, $is_avail, $is_correct,
                        $is_no_attempts, $questtype1_answer_incorrect, $got_one_correct, $c_quest_cur_attempt,
                        $c_all_attempts, $timer);
                    ...
                    if ($question[0]->c_partially_message) {
                        $jq_language['COM_QUIZ_PARTIALLY_CORRECT'] = htmlspecialchars(nl2br($question[0]->c_partially_message));
                    } elseif ($question[0]->c_wrong_message) {
                        $jq_language['COM_QUIZ_PARTIALLY_CORRECT'] = htmlspecialchars(nl2br($question[0]->c_wrong_message));
                    } else {
                        $jq_language['COM_QUIZ_PARTIALLY_CORRECT'] = $jq_language['COM_QUIZ_ANSWER_INCORRECT'];
                    }


                    if ($got_one_correct) {
                        if ($question[0]->c_partially_message) {
                            $jq_language['COM_QUIZ_ANSWER_INCORRECT'] = htmlspecialchars(nl2br($question[0]->c_partially_message));
                        }
                    }
                    ...
                    if ($cur_tmpl) {
                        ...
                        $ret_str .= $this->feedback($database, $blank_fbd, $blank_fbd_count, $answer, $quest_id,
                            $c_quest_cur_attempt, $c_all_attempts, $quiz, $question, $qtype, $is_no_attempts,
                            $is_correct, $jq_language, $got_one_correct, $questtype1_answer_incorrect, $is_avail);
        ...
        if ($cur_tmpl && ($quiz_id) && ($stu_quiz_id) && is_array($quest_ids) && count($quest_ids)) {
            ...
            if (($quiz_id == $start_quiz)) {
                ...
                for ($q = 0, $qn = count($quest_ids); $q < $qn; $q++) {
                    ...
                    $ret_str .= $this->feedback($database, $blank_fbd, $blank_fbd_count, $answer, $quest_id,
                        $c_quest_cur_attempt, $c_all_attempts, $quiz, $question, $qtype, $is_no_attempts, $is_correct,
                        $jq_language, $got_one_correct, $questtype1_answer_incorrect, $is_avail);
    ...
    public function JQ_FinishQuiz()
    {
        ...
        if (!empty($session_jq_result_mode)
            ...
            if (!empty($session_jq_result_mode_lid)
        ...
        if ($quiz_id) {
            ...
            if ($stu_quiz_id) {
                ...
                if ($my->id != $st_quiz_data[0]->c_student_id && !$is_share) {
                    return '';
                }
                ...
                if (!$result_mode) {
                    ...
                    if ($rel_id && $my->id && $user_passed) {
                        ...
                        if ($rel_id && $my->id && !$user_passed) {
                            ...
                            if (!$rel_id && $lid && $my->id && $user_passed) {
                                $query = "SELECT * FROM `#__quiz_lpath` WHERE `id` = '{$lid}' AND published = 1";
                                $database->SetQuery($query);
                                $lpath = $database->loadObjectList();
                                if (!empty($lpath)) {
                ...
                if ($cur_tmpl) {
                    ...
                    if ($quiz_info->c_resbycat == 1) {
                        ...
                        foreach ($q_cate as $curcat) {
                            if ($curcat[2] || $i) {
                                $percent = ($curcat[2]) ? number_format(($curcat[1] / $curcat[2]) * 100, 0, '.',
                                    ',') : 0;
                                $c_resbycat .= "<div class='jq_cat_score'>" . $curcat[0] . ': ' . sprintf(JText::_('COM_QUIZ_RES_MES_SCORE_TPL'),
                                        $curcat[1], $curcat[2], $percent) . "</div><br />";
                            }
                            $i++;
                        }
                    }
                    ...
                    if ($AUP_isset) {
                        ...
                        if ($result[0]->displaymsg == 1 && $result[0]->fixedpoints != 1) {
                            $results_txt = str_replace('<!-- SYSTEM MESSAGE CONTAINER -->',
                                sprintf(JText::_('COM_QUIZ_RES_MES_SCORE_MESS'),
                                    number_format($user_score, 2, '.', ' '), number_format($max_score, 2, '.', ' ')),
                                $results_txt);
                        } elseif ($result[0]->displaymsg == 1 && $result[0]->fixedpoints == 1) {
                            $results_txt = str_replace('<!-- SYSTEM MESSAGE CONTAINER -->', $result[0]->msg,
                                $results_txt);
                        }
                    }

                    $user_score_percent = ($max_score) ? ($user_score / $max_score) * 100 : 0;
                    $results_txt = str_replace('<!-- TOTAL USER SCORE -->',
                        sprintf(JText::_('COM_QUIZ_RES_MES_SCORE_TPL'), number_format($user_score, 2, '.', ' '),
                            number_format($max_score, 2, '.', ' '), number_format($user_score_percent, 2, '.', ' ')),
                        $results_txt);
                    $results_txt = str_replace('<!-- PASSING SCORE -->',
                        sprintf(JText::_('COM_QUIZ_RES_MES_PAS_SCORE_TPL'), number_format($nugno_score, 2, '.', ' '),
                            number_format($quiz_info->c_passing_score, 2, '.', ' ')), $results_txt);
                    ...
                    if ($c_resbycat) {
                        ...
                        $results_txt = JoomlaquizHelper::jq_substr($results_txt, 0,
                                $p1) . JoomlaquizHelper::jq_substr($results_txt, $p2 + 32);
                    }
                    ...
                    if ($c_manual || !$quiz_info->c_show_result) {
                        ...
                        if ($p1 && $p2) {
                            $results_txt = JoomlaquizHelper::jq_substr($results_txt, 0,
                                    $p1) . JoomlaquizHelper::jq_substr($results_txt, $p2 + 29);
                        }
                    }
                    ...
                    if ((!$is_share || $false_share) && $quiz->c_share_buttons) {
                        ...
                        $user_score_replaced = sprintf($sshare_message, number_format($user_score, 2, '.', ' '),
                                number_format($max_score, 2, '.', ' ')) . $quiz->c_title;
                    ...
                    if ($fin_message && (!$is_share || $false_share)) {
                        ...
                        $results_txt = JoomlaquizHelper::jq_substr($results_txt, 0,
                                $p1) . JoomlaquizHelper::jq_substr($results_txt, $p2 + 25);
                    }
                    ...
                    if ($rel_id) {
                        ...
                        $query->select('1')
                            ...
                            ->where($db->qn('c_quiz_id') . ' = ' . $db->q($quiz->c_id))
                            ->where($db->qn('c_student_id') . ' = ' . $db->q($my->id));
                        if (JoomlaquizHelper::isQuizAttepmts($quiz_id, 0, $rel_id, $package_id,
                                $tmp) && (!$quiz->one_time || !$db->setQuery($query)->loadResult())) {
                            $is_attempts = true;
                        ...
                        if ($lid) {
                            ...
                            $query->select('1')
                                ...
                                ->where($db->qn('c_quiz_id') . ' = ' . $db->q($quiz->c_id))
                                ->where($db->qn('c_student_id') . ' = ' . $db->q($my->id));
                            if (JoomlaquizHelper::isQuizAttepmts($quiz_id, $lid, 0, 0,
                                    $tmp) && (!$quiz->one_time || !$db->setQuery($query)->loadResult())) {
                                $is_attempts = true;
                            ...
                            $href = ($reStartString) ? /*JRoute::_('index.php'.$reStartString)*/
                                'index.php' . $reStartString
                    ...
                    $certAccessGranted = $this->user->authorise('core.certificate',
                    ...
                    if (strlen(implode('', $footer_ar)) > 7) {
                        ...
                        if ($quiz->c_certificate && !$c_manual && $user_passed && !$certAccessGranted) {
                            ...
                            if (!empty($msgDbText)) {
                                $results_txt = str_replace('<!-- QUIZ CERTIFICATE MESSAGE -->', $msgDbText,
                                    $results_txt);
                            } else {
                                $results_txt = str_replace('<!-- QUIZ CERTIFICATE MESSAGE -->',
                                    JText::_("COM_JOOMLAQUIZ_CERTIFICATE_MSG_FE"), $results_txt);
                            }
                        }
                    ...
                    if ((!$result_mode || $result_mode == 3) && $quiz->c_statistic && !$c_manual) {
                        if ($quiz->c_hide_feedback && !$user_passed && $is_attempts) {
                            ...
                            $results_txt = JoomlaquizHelper::jq_substr($results_txt, 0,
                                    $p1) . JoomlaquizHelper::jq_substr($results_txt, $p2 + 33);
                        } else {
                    ...
                    if (!$result_mode && $quiz->c_emails && $quiz->c_email_chk) {
                        if (($quiz->c_ifmanual && $c_manual) || !$quiz->c_ifmanual) {
                            ...
                            if (is_array($emails) && count($emails)) {
                                ...
                                $percent = ($max_score) ? number_format(($user_score / $max_score) * 100, 0, '.',
                                    ',') : 0;
                                if (!$c_manual) {
                                    $message .= JText::_('COM_QUIZ_RES_MES_SCORE2') . '  ' . sprintf(JText::_('COM_QUIZ_RES_MES_SCORE_TPL'),
                                            $user_score, $max_score, $percent) . "<br/>\n";
                                } else {
                                    $message .= JText::_('COM_QUIZ_RES_MES_SCORE2') . ' not graded yet ' . "<br/>\n";
                                }
                                $message .= JText::_('COM_QUIZ_RES_MES_PAS_SCORE') . '  ' . sprintf(JText::_('COM_QUIZ_RES_MES_PAS_SCORE_TPL'),
                                        $nugno_score, $quiz_info->c_passing_score) . "<br/>\n";
                                ...
                                foreach ($emails as $email) {
                                    ...
                                    $jmail->sendMail($mailfrom, $sitename, trim($email), $subject, $message, 1, null,
                                        null, null, null, null);
                                }
                            }
                        }
    ...
    public function JQ_StartReview()
    {
        ...
        if ($quiz_id) {
            ...
            if ($stu_quiz_id) {
                ...
                if ($my->id != $st_quiz_data[0]->c_student_id && !$my->authorise('core.managefe', 'com_joomlaquiz')) {
                    return '';
                }
                if ($start_quiz != $quiz_id) {
                    return '';
    ...
    public function JQ_NextReview()
    {
        ...
        if (($quiz_id) && ($stu_quiz_id) && !empty($quest_ids)) {
        ...
        if (($quiz_id == $q_quiz) && ($preview_id == $preview_code) && ($quest_id)) {
            ...
            if ($cur_tmpl) {
                ...
                if (!empty($q_data)) {
                    ...
                    $msg_html = JoomlaQuiz_template_class::JQ_show_messagebox('',
                        (($is_correct) ? (($qtype == 8) ? $jq_language['COM_QUIZ_ANSWER_ACCEPTED'] : $jq_language['COM_QUIZ_ANSWER_CORRECT']) : $jq_language['COM_QUIZ_ANSWER_INCORRECT']));
                    if ($qtype == 9) {
                        $msg_html = '';
                    }
            ...
            foreach ($quest_ids as $ii => $quest_id) {
                ...
                if ($cur_tmpl) {
                    if (!empty($q_data)) {
                        ...
                        if ($qtype == 1) {
                            $msg_html = JoomlaQuiz_template_class::JQ_show_messagebox('',
                                (($is_correct) ? ($questtype1_answer_incorrect ? $questtype1_answer_incorrect : $jq_language['COM_QUIZ_ANSWER_CORRECT']) : (($questtype1_answer_incorrect) ? ($questtype1_answer_incorrect) : ($jq_language['COM_QUIZ_ANSWER_INCORRECT']))));
                        }
            ...
            for ($q = 0, $qn = count($quest_ids); $q < $qn; $q++) {
                ...
                if ($cur_tmpl) {
                    ...
                    if (!$quiz->c_show_qfeedback && !$is_no_attempts && ($quiz->c_feedback && $question[0]->c_feedback) && $qtype != 9 && !$question[0]->c_immediate) {
                        $msg_html = JoomlaQuiz_template_class::JQ_show_messagebox('',
                            (($is_correct) ? (($qtype == 8) ? $jq_language['COM_QUIZ_ANSWER_ACCEPTED'] : $jq_language['COM_QUIZ_ANSWER_CORRECT']) : ($got_one_correct ? $jq_language['COM_QUIZ_PARTIALLY_CORRECT'] : $jq_language['COM_QUIZ_ANSWER_INCORRECT'])));
                        if ($qtype == 1) {
    ...
    public function JQ_GetQuestData($q_data, $i_quiz_id, $stu_quiz_id = 0)
    {
        ...
        if (!$seek_quest_id && $stu_quiz_id && is_array($all_quests) && !empty($all_quests) && is_array($qchids) && !empty($qchids)) {
            $query = "SELECT c_question_id FROM #__quiz_r_student_question WHERE c_stu_quiz_id = '" . $stu_quiz_id . "'";
            $database->SetQuery($query);
            $q_ids = $database->loadColumn();
            ...
            if (is_array($q_ids) && !empty($q_ids)) {
                ...
                if (!empty($diff)) {
                    if (count($diff) == 1 && array_pop($diff) == $q_data->c_id) {
                        $is_last = 1;
                    }
                } else {
                    $is_last = 1;
        ...
        } elseif (!$seek_quest_id && is_array($all_quests) && !empty($all_quests) && is_array($qchids) && !empty($qchids)) {
    ...
    public function JQ_GetPanelData($quiz_id, $q_data, $stu_quiz_id = 0)
    {
        ...
        foreach ($panel_data as $panel_row) {
            ...
            $panel_row->c_question = (strlen($panel_row->c_question) > 200 ? JoomlaquizHelper::jq_substr($panel_row->c_question,
                    0, 160) . '...' : $panel_row->c_question);
            $panel_str .= JoomlaQuiz_template_class::JQ_panel_data($panel_row, $all_quests, $cquests, $stu_quiz_id, $k,
                $n);
            $k = 3 - $k;
            $n++;
        }
    ...
    public function JQ_GetStatistic($quiz_id, $stu_quiz_id, $c_show_qfeedback = 0)
    {
        ...
        if (is_array($q_ids) && is_array($info)) {
            foreach ($q_ids as $z => $q_id) {
                ...
                $q_data = $database->LoadObjectList();
                if (isset($q_data[0])) {
                    $ret_questions .= $this->JQ_GetQuestData_Feedback($q_data[0], $quiz_id, $stu_quiz_id,
                            ($z + $limitstart + 1),
                            $c_show_qfeedback) . JoomlaQuiz_template_class::JQ_getFeedbackQuestionDelimeter();
                }
            }
        }
        ...
        $quest_pp = JHTML::_('select.genericlist', $qpp, 'quest_per_page',
            'class="inputbox" size="1" id="quest_per_page" ' . $javascript, 'value', 'text', $quest_per_page);
    ...
    public function JQ_GetQuestData_Feedback($q_data, $i_quiz_id, $stu_quiz_id, $no = '', $c_show_qfeedback = 0)
    {
        ...
        if ($cur_template) {
            ...
            if ($q_data->c_type == 6) {
                $q_data->c_question = JoomlaquizHelper::Blnk_replace_quest_fdb($q_data->c_id, $q_data->c_question,
                    $stu_quiz_id);
            }
    ...
    function JQ_GetQuestData_review($q_data, $i_quiz_id, $stu_quiz_id = 0)
    {
        ...
        $query = "SELECT c_id, ordering FROM `#__quiz_t_question` WHERE  c_id IN ('" . implode("','",
                $qchids) . "') AND published = 1 ORDER by ordering, c_id";
        $database->SetQuery($query);
        $tmp = $database->loadObjectList();
        $all_quiz_quests = array();
        foreach ($qchids as $qchid) {
        ...
        $query = "SELECT * FROM `#__quiz_t_question` WHERE  `ordering` >= '{$last_ordering}' AND published = 1 AND c_id <> '{$last_pbreak_id}' AND c_id IN ('" . implode("','",
                $q_ids) . "') ORDER by ordering, c_id";
        $database->SetQuery($query);
        $tmp = $database->loadObjectList();
        $all_quests = array();
        foreach ($qchids as $qchid) {
        ...
        foreach ($all_quests as $q_data) {
            ...
            if ($cur_template) {
                ...
                ob_end_clean();
                if ($q_data->c_type == 6) {
                    $ret_str .= '<quest_data><![CDATA[<div>' . JoomlaquizHelper::Blnk_replace_quest_review($q_data->c_id,
                            $q_data->c_question) . '</div>]]></quest_data>';
                } else {
