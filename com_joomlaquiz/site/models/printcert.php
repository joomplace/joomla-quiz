...
{	
	...
	public function JQ_printCertificate(){
		...
		if (count($stu_quiz)) {
			$stu_quiz = $stu_quiz[0];
			if ( ($my->id == $stu_quiz->c_student_id || $my->authorise('core.manage','com_joomlaquiz')) && ($user_unique_id == $stu_quiz->unique_id) ) {
	...
	function write_multiline_text ($image, $font_size, $start_x, $start_y, $color, $font, $grey, $shadow, $text, $max_width) { 
		...
		for($i = 0; $i < count($words); $i++) { 
			...
			if($dim[4] < ($max_width - $start_x)) { 
				...
				if ($shadow) imagettftext($image, $font_size, 0, $start_x, $start_y, $grey, $font, $string);
