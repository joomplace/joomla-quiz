...
if(isset($lpath->error) && $lpath->error){
	...
	<div class="contentpane joomlaquiz">
		<h1 class="componentheading"><?php echo $lpath->title;?></h1>
		<br/>
		...
		<div class="jq_lpath_container">
			<?php foreach($lpath_all as $k=>$lpath_item) { ?>
				...
				<div class="jq_lpath_step_descr<?php echo ($lpath_item->show_link && isset($lpath_all[$k+1])&&!$lpath_all[$k+1]->show_link?'':'')?>">
					...
					if ($lpath_item->show_link  && $lpath_item->type != 'a') {
						...
						<div id="jq_start_link_container" style="float:right; margin-right:15px; line-height: 32px; text-transform: uppercase; width: auto !important; background-position: right center !important;">
							<a style="padding-right:40px;" href="<?php echo JRoute::_('index.php?option=com_joomlaquiz&view=quiz'.($lpath->rel_id? '&package_id='.$lpath->package_id.'&rel_id='.$lpath->rel_id: '&lid='.$lpath->id).'&'.$item_id_name.'='.$lpath_item->all_id.JoomlaquizHelper::JQ_GetItemId());?>"><?php echo JText::_('COM_QUIZ_START');?> Exam</a>
						</div><br /><br />
						<hr>
					<?php }?>
				</div>
