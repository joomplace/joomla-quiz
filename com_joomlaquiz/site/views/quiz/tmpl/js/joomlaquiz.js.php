...
function jq_UpdateTaskDiv(task, skip_question) {
	...
	switch (task) {
		...
		case 'prev_next':
			...
			<?php } else {?>
			task_container = jq_NextButton('jq_QuizNextOn()', '<?php echo addslashes(JText::_('COM_QUIZ_NEXT'))?>')+''<?php if ($quiz->c_enable_prevnext) {?>+jq_PrevButton('jq_QuizPrevQuestion()','<?php echo addslashes(JText::_('COM_QUIZ_PREV'))?>')<?php }?>;
			<?php } ?>
		break;
