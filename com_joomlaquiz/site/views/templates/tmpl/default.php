...
EOF_MSG;
    ...
    public static function JQ_createBlank($blank_id, $blank_value = '', $css_class = '', $blk_id = 0, $qform = 0, $q_id = '')
    { //html template for 'Blank' questions
        if ($qform) {
    ...
    public static function JQ_createBlank_review($review_val = '', $color = 'red')
    { //html template for review 'Blank' questions
        $jq_tmpl_html = "<font color='{$color}' class='jq_blank'>{" . $review_val . "}</font> \n";
        return $jq_tmpl_html;
    }

    public static function JQ_createBlank_fdb(&$correct_answers, $user_answer = '', $color = 'red', $is_correct)
    { //html template for final feedback 'Blank' questions
        $tag = JFactory::getLanguage()->getTag();
        $lang = JFactory::getLanguage();
        $lang->load('com_joomlaquiz', JPATH_SITE, $tag, true);
