...
<?php echo $this->loadTemplate('menu'); ?>
    <form action="<?php echo JRoute::_('index.php?option=com_joomlaquiz&view=products'); ?>" method="post"
            ...
            <div style="margin-top: 10px;" id="j-main-container">
                ...
                <div class="btn-group pull-right">
                    <label for="category_id"
                           class="element-invisible"><?php echo JText::_('COM_JOOMLAQUIZ_USERNAME2'); ?></label>
                ...
                <table class="table table-striped" id="productList">
                    ...
                    <tr>
                        ...
                        <th>
                            <?php echo JHtml::_('grid.sort', 'COM_JOOMLAQUIZ_PRODUCTS', 'name', $listDirn, $listOrder); ?>
                        </th>
                        <th>
                            <?php echo JHtml::_('grid.sort', 'COM_JOOMLAQUIZ_PRODUCT_SKU', 'product_sku', $listDirn, $listOrder); ?>
                        </th>
                        <th>
                            <?php echo JHtml::_('grid.sort', 'COM_JOOMLAQUIZ_CATEGORY_NAME', 'category_name', $listDirn, $listOrder); ?>
                        </th>
                    </tr>
                    </thead>
                    <tfoot>
                    ...
                    <?php if (sizeof($this->items)) {
                        foreach ($this->items as $i => $item) :
                            $link = JRoute::_('index.php?option=com_joomlaquiz&task=product.edit&pid=' . $item->pid);
                            $canEdit = $user->authorise('core.edit', $extension . '.products.' . $item->pid);
                            $canCheckin = $user->authorise('core.admin', 'com_checkin');
                            $canChange = $user->authorise('core.edit.state', $extension . '.products.' . $item->pid) && $canCheckin;
                            ?>
                            <tr class="row<?php echo $i % 2; ?>" sortable-group-id="1">
                                <td class="order nowrap center">
                                    <?php echo($i + 1); ?>
                                    <input type="text" style="display:none" name="pid" size="5"
                                           value="<?php echo $item->pid; ?>" class="width-20 text-area-order "/>
                                </td>
                                <td class="center">
                                    <?php echo JHtml::_('grid.id', $i, $item->pid); ?>
                                </td>
                                <td class="nowrap has-context">
                                    <div class="pull-left">
                                        <a href="<?php echo $link; ?>">
                                            <?php echo $item->name ? $item->name : $item->product_name; ?>
                                        </a>
                                    </div>
                                </td>
                    ...
                    <?php } else { ?>
                        <tr>
                            <td colspan="10" align="center">
                                <?php echo JText::sprintf('COM_JOOMLAQUIZ_NOPRODUCTSCREATED'); ?>
                                <a href="<?php echo JRoute::_('index.php?option=com_joomlaquiz&task=product.add'); ?>">
