...
<form action="<?php echo JRoute::_('index.php?option=com_joomlaquiz&layout=edit&id=' . (int)$this->item->id); ?>"
      enctype="multipart/form-data" method="post" name="adminForm" id="payment-form" class="form-validate">
    <div id="j-main-container" class="span10 form-horizontal">
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (strval($row->pid) != strval($product_id)) {
                ...
                <legend><?php echo $row->product_title; ?></legend>
            ...
            <tr class="<?php echo "row$k"; ?>">
                ...
                <td nowrap class="has-context"><?php
                    if ($display == 0 || $display == 1) {
                        ?>
                        <input type="checkbox" name="xdays[]" value="<?php echo $row->id; ?>"
                               checked/>&nbsp;&nbsp;<?php echo JText::_('COM_JOOMLAQUIZ_XDAYS'); ?>&nbsp;&nbsp;
                    <?php }
                    if ($display == 0 || $display == 2) {
                        ?>
                        <input type="checkbox" name="period[]" value="<?php echo $row->id; ?>"
                               checked/>&nbsp;&nbsp;<?php echo JText::_('COM_JOOMLAQUIZ_PERIOD'); ?>&nbsp;&nbsp;<?php
                    } ?>
                    <br/><input type="checkbox" name="attempts[]" value="<?php echo $row->id; ?>"
                                checked/>&nbsp;&nbsp;<?php echo JText::_('COM_JOOMLAQUIZ_ATTEMPTS_NO'); ?>
                </td>
            </tr>
            <?php
