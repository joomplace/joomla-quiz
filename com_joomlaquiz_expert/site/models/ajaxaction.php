...
class JoomlaquizModelAjaxaction extends JModelList
{
    ...
    public function JQ_StartQuiz() {
        ...
        $query->select($db->qn('qtq') . '.*')
            ...
            ->where($db->qn('qtq.c_id') . ' = ' . $db->q($quiz_id))
        ;
//		$query = "SELECT * FROM #__quiz_t_quiz WHERE c_id = '".$quiz_id."'";
        $db->SetQuery ($query );
        $quiz = $db->LoadObjectList();
        ...
        if ($quiz_id) {
            ...
            if (!$stu_quiz_id) {
                ...
                $query = "INSERT INTO #__quiz_r_student_quiz (c_order_id, c_rel_id, c_lid, c_quiz_id, c_student_id, c_total_score, c_total_time, c_date_time, c_passed, unique_id, unique_pass_id, c_finished, user_email, user_name, user_surname, params)"
                    . "\n VALUES('".$package_id."', '".$rel_id."', '".$lid."', '".$quiz_id."', '".$my->id."', '0', '0', '".$quiz_time."', '0', '".$user_unique_id."', '".$unique_pass_id."', 0, '".$user_email."', '".$user_name."', '".$user_surname."', ".$db->quote($cust_params).")";
                $db->SetQuery($query);
                $db->query();
                $stu_quiz_id = $db->insertid();
                ...
                if($rel_id && $my->id) {
                    if ($package_id < 1000000000) {
                        if($product_type == 'vm'){
                            ...
                            ;
                        } else if($product_type == 'j2s'){
                            $query = $db->getQuery(true);
                            $query->select($db->qn('qp') . '.*')
                                ->select($db->qn('jo.created_on', 'xdsys_start'))
                                ->from($db->qn('#__quiz_products', 'qp'))
                                ->innerJoin($db->qn('#__j2store_orderitems', 'joi')
                                    . ' ON '
                                    . $db->qn('joi.product_id') . ' = ' . $db->qn('qp.pid')
                                )
                                ->innerJoin($db->qn('#__j2store_orders', 'jo')
                                    . ' ON '
                                    . $db->qn('jo.cart_id') . ' = ' . $db->qn('joi.cart_id')
                                )
                                ->where($db->qn('qp.id') . ' = ' . $db->q($rel_id))
                                ->where($db->qn('jo.j2store_order_id') . ' = ' . $db->q($package_id))
                                ->where($db->qn('jo.user_id') . ' = ' . $db->q($my->id))
                                ->where($db->qn('jo.order_state_id') . ' = 1')
                            ;
                        } else if($product_type == 'eb'){
                            $query = $db->getQuery(true);
                            $query->select($db->qn(qp) . '.*')
                                ->select($db->qn('ebr.payment_date', 'xdsys_start'))
                                ->from($db->qn('#__quiz_products', 'qp'))
                                ->innerJoin($db->qn('#__eb_registrants', 'ebr')
                                    . ' ON '
                                    . $db->qn('ebr.event_id') . ' = ' . $db->qn('qp.pid')
                                )
                                ->where($db->qn('qp.id') . ' = ' . $db->q($rel_id))
                                ->where($db->qn('ebr.id') . ' = ' . $db->q($package_id))
                                ->where($db->qn('ebr.user_id') . ' = ' . $db->q($my->id))
                                ->where($db->qn('ebr.published') . ' = 1')
                            ;
                        }
                    } else {
            ...
            switch($quiz->c_pool)
            {
                ...
                case '2':	$query = "SELECT * FROM #__quiz_pool WHERE q_id = '".$quiz_id."'";
                    $db->SetQuery($query);
                    $poolcat_data = $db->LoadObjectList();
    ...
    public function JQ_NextQuestion($user_time = 0) {
        ...
        if ($cur_tmpl && ($quiz_id) && ($stu_quiz_id) && is_array($quest_ids) && count($quest_ids)) {
            ...
            $db->SetQuery($query);
            $st_quiz_data = $db->LoadObjectList();
            ...
            if ( ($quiz_id == $start_quiz) ) {
                ...
                for($q=0, $qn = count($quest_ids); $q < $qn; $q++) {
                    ...
                    $quiz_time1 = strtotime(JHtml::_('date',time(),'Y-m-d H:i:s'));
                    $db->setQuery("SELECT `c_time_limit` FROM `#__quiz_t_question` WHERE `c_id` = '".$quest_id."'");
                    $limit_time = $db->loadResult();
                    $quiz_time2a = strtotime($quiz_time2);
                    $user_time = $quiz_time1 - $quiz_time2a;
                    ...
                    if ($cur_tmpl) {
                        ...
                        if(!$is_correct){
                            ...
                            $db->SetQuery( $query );
                            $c_detailed_feedback = '</br>'.$db->LoadResult();
                        }
                ...
                if($qch_ids) {
                    ...
                    if(count($qchids)){
                        ...
                        for($i = $quest_pos; $i < $q_total; $i++){
                            $db->setQuery("SELECT `c_attempts` FROM #__quiz_t_question WHERE `c_id` = '".$qchids[$i]."'");
                            $all_attempts = $db->loadResult();

                            $query = "SELECT `c_attempts` FROM #__quiz_r_student_question WHERE `c_stu_quiz_id` = '".$stu_quiz_id."' AND `c_question_id` = '".$qchids[$i]."'";
                            $db->SetQuery( $query );
                            $c_attempts = $db->LoadResult();
                ...
                if (isset($q_data[$j])) {
                    ...
                    $db->SetQuery( $query );
                    $q_total_score = $db->LoadResult();

                    $query = "SELECT c_date_time FROM #__quiz_r_student_quiz WHERE c_id = '".$stu_quiz_id."'";
                    $db->SetQuery( $query );
                    $q_beg_time = $db->LoadResult();
    ...
    public function JQ_FinishQuiz() {
        ...
        if($share_id){
            $db->setQuery("SELECT COUNT(id) FROM `#__quiz_r_student_share` WHERE `id` = '".$share_id."'");
        ...
        if ($quiz_id) {
            ...
            $db->setQuery("SELECT `user_email`, `user_name` FROM `#__quiz_r_student_quiz` WHERE `c_id` = '".$stu_quiz_id."'");
            $stu_quiz_data = $db->loadObject();
            ...
            if($quiz->c_share_buttons){
                if($is_share && !$false_share){
                    ...
                    $db->setQuery("SELECT `c_stu_quiz_id` FROM `#__quiz_r_student_share` WHERE `id` = '".$share_id."'");
                    $stu_quiz_id = $db->loadResult();

                    $db->setQuery("SELECT `unique_id` FROM `#__quiz_r_student_quiz` WHERE `c_id` = '".$stu_quiz_id."'");
                    $user_unique_id = $db->loadResult();
                    ...
                    $db->setQuery("SELECT COUNT(`id`) FROM `#__quiz_r_student_share` WHERE `c_share_id` = '".$c_share_id."'");
                    $is_share_exists = $db->loadResult();

                    if(!$is_share_exists){
                        $db->setQuery("INSERT INTO `#__quiz_r_student_share` (`id`, `c_quiz_id`, `c_stu_quiz_id`, `c_user_id`, `c_share_id`) VALUES ('', '".$quiz_id."', '".$stu_quiz_id."', '".$my->id."', '".$c_share_id."')");
                        $db->execute();

                        $share_id = $db->insertid();
                    } else {
                        $db->setQuery("SELECT `id` FROM `#__quiz_r_student_share` WHERE `c_share_id` = '".$c_share_id."'");
                        $share_id = $db->loadResult();
                    }
            ...
            if ($stu_quiz_id) {
                ...
                $query = $db->getQuery(true);
                $query->select(1);
                $query->from($db->qn('#__quiz_t_question', 'q'));
                $query->from($db->qn('#__quiz_r_student_question', 'sq'));
                $query->where($db->qn('q.c_id').' IN ('.$qch_ids.')');
                $query->where($db->qn('q.published').' = '.$db->q('1'));
                $query->where($db->qn('q.c_manual').' = '.$db->q('1'));
                $query->where($db->qn('q.c_id').' = '.$db->qn('sq.c_question_id'));
                $query->where($db->qn('sq.c_stu_quiz_id').' = '.$db->q($stu_quiz_id));
                $query->where($db->qn('sq.reviewed').' = '.$db->q('0'));

                $db->SetQuery( $query );
                $c_manual = (int)$db->LoadResult();
                ...
                if(!$result_mode) {
                    $quiz_time1 = strtotime(JHtml::_('date',time(),'Y-m-d H:i:s'));
                    $db->setQuery("SELECT `c_time_limit` FROM `#__quiz_t_quiz` WHERE `c_id` = '".$quiz_id."'");
                    $limit_time = $db->loadResult();
                    $quiz_time2a = strtotime($quiz_time2);
                    $user_time = $quiz_time1 - $quiz_time2a;
                ...
                if ($cur_tmpl) {
                    ...
                    if (strlen(implode('', $footer_ar)) > 7){
                        ...
                        if($quiz->c_certificate && !$c_manual && $user_passed && !$certAccessGranted){
                            ...
                            $db->setQuery('SELECT c_quiz_certificate_access_message FROM #__quiz_t_quiz WHERE c_id='.$quiz->c_id);
                    ...
                    if ($rows[0]->c_email_to) {
                        ...
                        if ($c_manual) {
                            if (!preg_match("/^[_\.0-9a-z-]+@([0-9a-z][0-9a-z-]+\.)+[a-z]{2,3}$/", $email_address)) {
                                ...
                                $query = "UPDATE #__quiz_r_student_quiz SET user_email = ".$db->Quote($email_address)." WHERE c_id = ".$stu_quiz_id;
                                $db->SetQuery( $query );
                                $db->execute();
                                $result = 3;
                            }
    ...
    function JQ_ShowPage() {
        ...
        $quiz_id = intval( JFactory::getApplication()->input->get( 'quiz', 0 ) );
        $db->setQuery("SELECT `c_show_qfeedback` FROM `#__quiz_t_quiz` WHERE `c_id` = '".$quiz_id."'");
        $c_show_qfeedback = $db->loadResult();
        $stu_quiz_id = intval( JFactory::getApplication()->input->get( 'stu_quiz_id', 0 ) );
    ...
    public function JQ_GetQuestData($q_data, $i_quiz_id, $stu_quiz_id = 0) {
        ...
        $db->SetQuery("SELECT a.template_name FROM #__quiz_templates as a, #__quiz_t_quiz as b WHERE b.c_id = '".$i_quiz_id."' and b.c_skin = a.id");
        ...
        foreach($all_quests as $q_data) {
            ...
            if ($cur_template) {
                ...
                $db->SetQuery( $query );
                $c_flag_question = ($db->LoadResult()) ? $db->LoadResult() : 0;
    ...
    public function JQ_GetNextQuestion($qch_ids, $quiz_id, $stu_quiz_id, $current_quest)
    {
        $quest_id = 0;
        $db = JFactory::getDBO();
        $db->setQuery("SELECT * FROM `#__quiz_t_quiz` WHERE `c_id` = '".$quiz_id."'");
        $quiz_data = $db->loadObject();
    ...
    public function JQ_GetPanelData($quiz_id, $q_data, $stu_quiz_id=0) {
        ...
        $db = JFactory::getDBO();
        $db->setQuery("SELECT qt.template_name FROM #__quiz_templates as qt LEFT JOIN #__quiz_t_quiz as q ON q.c_skin = qt.id WHERE q.c_id = '".$quiz_id."'");
        $tmpl = $db->loadResult();
        ...
        if ($stu_quiz_id){
            ...
            $db->SetQuery( $query );
            $cquests = (array)$db->loadColumn();

            $query = "SELECT c_question_id FROM #__quiz_r_student_question WHERE c_stu_quiz_id = '".$stu_quiz_id."' ";
            $db->SetQuery( $query );
            $all_quests = (array)$db->loadColumn();
        }
    ...
    public function JQ_GetQuestData_Feedback($q_data, $i_quiz_id, $stu_quiz_id, $no = '', $c_show_qfeedback = 0) {
        ...
        if ($cur_template) {
            ...
            $db->SetQuery( $query );
            $max_points = (floatval($db->LoadResult()) + $q_data->c_point);
    ...
    function JQ_GetQuestData_review($q_data, $i_quiz_id, $stu_quiz_id=0) {
        ...
        foreach($all_quests as $q_data) {
            ...
            if ($cur_template) {
                ...
                $query = "SELECT SUM(a_point) FROM #__quiz_t_choice WHERE c_question_id = '".$q_data->c_id."' AND c_right = 1";
                $db->SetQuery( $query );
                $q_data->c_point = $q_data->c_point.' - '.(floatval($db->LoadResult()) + $q_data->c_point);
