    ...
    <div class="contentpane joomlaquiz">
        ...
        <div style="float:right; padding-right:15px;">
            <a href="javascript: void(0);"
               onclick="javascript: jq_jQuery('.jq_lpath_container .jq_lpath_step_descr').show('150');"><?php echo JText::_('COM_QUIZ_EXPAND_ALL'); ?></a>&nbsp;&nbsp;
            <a href="javascript: void(0);"
               onclick="javascript: jq_jQuery('.jq_lpath_container .jq_lpath_step_descr').hide('150');"><?php echo JText::_('COM_QUIZ_COLLAPSE_ALL'); ?></a>
        </div>
        <br/>
            ...
            <?php foreach ($lpath_all as $k => $lpath_item) { ?>
                <div class="jq_lpath_step_title">
                    ...
                    <?php if (false && $lpath_item->short_description) { ?>
                        <div class="jq_lpath_step_expand" style="float:right;">
                            [<?php echo JText::_('COM_JQ_SHOW_DESCRIPTION'); ?>]
                        </div>
                    <?php } ?>
                </div>
                <div class="jq_lpath_step_descr<?php echo($lpath_item->show_link && isset($lpath_all[$k + 1]) && !$lpath_all[$k + 1]->show_link ? ' expanded' : '') ?>">
                    ...
                    if ($lpath_item->show_link) {
                        ...
                        <br/>
                        <div id="jq_start_link_container"
                             style="float:right; margin-right:15px; line-height: 32px; text-transform: uppercase; width: auto !important; background-position: right center !important;">
                            <a style="padding-right:40px;"
                               href="<?php echo JRoute::_('index.php?option=com_joomlaquiz&view=quiz' . ($lpath->rel_id ? '&package_id=' . $lpath->package_id . '&rel_id=' . $lpath->rel_id : '&lid=' . $lpath->id) . '&' . $item_id_name . '=' . $lpath_item->all_id . JoomlaquizHelper::JQ_GetItemId()); ?>"><?php echo JText::_('COM_QUIZ_START'); ?></a>
                        </div><br/><br/>
                        <hr>
                    <?php } ?>
                </div>
