...
function jq_MakeRequest(url, do_clear, silent) {
    ...
    quiz_blocked == 1;
    http_request.onreadystatechange = function () {
        jq_AnalizeRequest(http_request);
    };
            ...
            switch (task) {
                ...
                case 'time_is_up':
                    ...
                    for (var i = 0; i < children.length; i++) {
                        quiz_cont.removeChild(quiz_cont.childNodes[i]);
                    }
                    ;
...
function jq_Start_Question_TickTack(limit_time) {
    ...
    if (quest_timer_sec > 0) {
        ...
        if (quest_timer_min < 0) {
            quest_timer_min = quest_timer_min * (-1);
        }
        if (plus_sec < 0) {
            plus_sec = plus_sec * (-1);
        }
...
function JQ_gotoQuestion(qid) {
    jq_MakeRequest('&ajax_task=goto_quest&quiz=<?php echo $quiz->c_id?>&stu_quiz_id=' + stu_quiz_id + '&seek_quest_id=' + qid, 1);
}
...
function setFlag(qid) {
    ...
    jq_jQuery.ajax({
        ...
        data: {quiz_id:, quest_id: qid, stu_quiz_id: stu_quiz_id, flag_quest: f_checked},
...
function jq_UpdateTaskDiv(task, skip_question) {
    ...
    switch (task) {
        ...
        case 'prev_next_last':
            ...
            task_container = jq_SubmitButton('jq_QuizNextOn()', '<?php echo addslashes(JText::_('COM_QUIZ_FINISH'))?>') +
                jq_PrevButton('jq_QuizPrevQuestion()', '<?php echo addslashes(JText::_('COM_QUIZ_PREV'))?>') + '';
    ...
    if (skip_question && !is_last && !skip_type) {
        task_container = '<a onclick="javascript: JQ_gotoQuestionOn(' + skip_question + ')" class="btn btn-primary" id="jq_skip_link" title="<?php echo addslashes(JText::_('COM_QUIZ_SKIP'))?>"><?php echo addslashes(JText::_('COM_QUIZ_SKIP'))?></a>' + task_container;
    } else if (skip_question && !is_last && skip_type && quest_type != 9) {

        task_container = '<a onclick="javascript:jq_QuizNextFinish()" id="jq_finish_link_container" class="btn btn-primary" title="<?php echo addslashes(JText::_('COM_QUIZ_FINISH'))?>"><?php echo addslashes(JText::_('COM_QUIZ_FINISH'))?></a><a class="btn btn-primary" onclick="javascript: JQ_gotoQuestionOn(' + skip_question + ')" id="jq_skip_link" title="<?php echo addslashes(JText::_('COM_QUIZ_SKIP'))?>"><?php echo addslashes(JText::_('COM_QUIZ_SKIP'))?></a>' + task_container;
...
function jq_SubmitButton(task, text) {
    ...
    return "<a class=\"jq_submit_link_container\" title=\"" + text + "\" onclick=\"" + task + "\" id=\"jq_submit_link_container\" href=\"javascript: void(0)\">" + text<?php if($quiz->template_name != 'joomlaquiz_pretty_blue'){?> + "<i class=\"jq_next_arrow\"></i>" + "</a>";
