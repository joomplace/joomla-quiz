...
class modCompletedlpHelper
{
    ...
    public static function getResult()
    {
        ...
        $query->select($db->qn('lpid'))
            ->from($db->qn('#__quiz_lpath_stage'))
            ->where($db->qn('stage') . ' = 0');
        $db->SetQuery($query);
        $not_completed = $db->loadColumn();
        ...
        $query->select(array('ql.id', 'ql.title AS title', 'c.title AS category'))
            ...
            ->where($db->qn('qls.uid') . ' = ' . $user_id . ' AND ' . $db->qn('ql.published') . ' = 1' . ' AND ' . $db->qn('qls.stage') . ' = 1')
            ->where($db->qn('qls.lpid') . ' NOT IN ( ' . implode(",", $not_completed) . ') GROUP BY qls.lpid');
        $db->SetQuery($query);
