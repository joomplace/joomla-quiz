...
$query->clear();
$query->select('*')
    ->from($db->qn('#__quiz_t_text'))
    ->where($db->qn('c_blank_id') .'='. $db->q((int)$blank_data[$z]->c_id))
    ->order($db->qn('ordering'));
$db->setQuery($query);
$ans_row = $db->loadObjectList();
?>
<div class="option-row">
    <table cellpadding="10" cellspacing="10" style="margin-top:15px;">
        <tr>
            <td><strong><?php echo JText::_('COM_JOOMLAQUIZ_BLANK');?>
                    <span id="blnk_num_<?php echo $z; ?>"><?php echo $z+1?></span></strong>
                <?php echo JText::_('COM_JOOMLAQUIZ_CODE_FORQUESTION');?>{blank<?php echo $z+1?>}
                <br />
                <?php echo JText::_('COM_JOOMLAQUIZ_CODE_FORQUESTION_BLANK_ALL_ANSWERS');?>{answers<?php echo $z+1?>}
            </td>
            <td>
    ...
    <span>&nbsp;&nbsp;<?php echo JText::_('COM_JOOMLAQUIZ_POINTS');?>&nbsp;</span><input type="text" id="new_points_<?php echo $z;?>" name="jq_hid_points_<?php echo $z;?>" value="<?php echo (int)$blank_data[$z]->points; ?>" size="5" class="text_area" />&nbsp;&nbsp;
    <span><?php echo JText::_('COM_JOOMLAQUIZ_CUSTOMCSS_CLASS');?>&nbsp;</span><input type="text" id="new_css_<?php echo $z;?>" name="jq_hid_css_<?php echo $z; ?>" value="<?php echo htmlspecialchars($blank_data[$z]->css_class, ENT_QUOTES, 'UTF-8'); ?>" size="20" class="text_area" />
    &nbsp;&nbsp;<label style="display:inline;"><input onchange="javascript: jq_changeCheckbox2(this, <?php echo $jq_num;  ?>);" type="checkbox" value="1" <?php echo (int)$blank_data[$z]->gtype ? ' checked="checked" ' : ''?> name="jq_hid_gtype_chk_<?php echo $z; ?>[]" />
        &nbsp;<?php echo JText::_('COM_JOOMLAQUIZ_POINTS_FOR_EACHANSWER');?></label>
    <input  type="hidden" name="jq_hid_gtype_<?php echo $z?>[]"; id="jq_hid_gtype_<?php echo $jq_num;  ?>" value="<?php echo (int)$blank_data[$z]->gtype? '1': '0'?>" />
    <?php $jq_num++;?>
    <br/><br/>
    <table class="table table-striped" id="qfld_tbl_<?php echo $z;?>" cellpadding="10" cellspacing="10">
        ...
        <tr class="<?php echo "row$k"; ?>">
            ...
            <td align="left">
                ...
                <textarea name="jq_hid_fields_<?php echo $z?>[]" style="display:none;"><?php echo htmlspecialchars($frow->c_text, ENT_QUOTES, 'UTF-8'); ?></textarea>
                <input type="hidden" name="jq_hid_fields_ids_<?php echo $z; ?>[]" value="<?php echo (int)$frow->c_id; ?>" />
            </td>
    ...
    <div class="adminform" style="margin-top:30px;">
        <div style="margin-left: 10px;">
            <?php echo '<span style="font-weight:bold;" class="distractors-title">'. JText::_('COM_JOOMLAQUIZ_DISTRACTORS') .' <span>'. ($z+1) . '</span></span>'; ?>
        </div>
        <table class="adminlist" id="qfld_tbl_fake<?php echo $z+1; ?>" cellpadding="7">
            <tr>
                <th width="20px" align="center"><?php echo JText::_('COM_JOOMLAQUIZ_SHARP'); ?></th>
            ...
            <?php
            $k = 0; $ii = 1;
            if(isset($fake_data[(int)$blank_data[$z]->c_id])){
                foreach ($fake_data[(int)$blank_data[$z]->c_id] as $frow) { ?>
                    <tr class="<?php echo "row$k"; ?>">
                        <td align="center"><?php echo $ii?></td>
                        <td align="left">
                            <?php echo htmlspecialchars($frow->c_text, ENT_QUOTES, 'UTF-8'); ?>
                            <input type="hidden" name="jq_hid_fake_<?php echo $z; ?>[]" value="<?php echo htmlspecialchars($frow->c_text, ENT_QUOTES, 'UTF-8'); ?>" />
                        </td>
        ...
        <table class="adminlist"  style="width:100%">
            <tr><th class="title" style="text-align:left;"><?php echo JText::_('COM_JOOMLAQUIZ_ADDNEW_DISTRACTOR');?></th></tr>
            <tr><td><div>
                        <input id="new_field_fake_<?php echo $z; ?>" class="text_area" style="width:205px " type="text" name="new_field_fake" />&nbsp;&nbsp;
                        <input class="modal-button btn" type="button" name="add_new_field" value="Add"
                               onclick="javascript:Add_new_tbl_field2(<?php echo $z; ?>);"/>
                    </div></td></tr>
        </table>
    </div>
    <hr/>
