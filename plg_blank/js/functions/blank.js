...
function createDD(){
    ...
    jq_jQuery(".jq_blank_droppable").each(function () {
        jq_jQuery(this).droppable({
            accept: function(el){
                return (jq_jQuery(el[0]).hasClass('jq_blank_draggable') && el[0].id.split('_blk_id_')[1] == this.id.split('blk_id_')[1])
                    || (jq_jQuery(el[0]).hasClass('jq_draggable_answer') && el[0].dataset.blank == this.id.split('blk_id_')[1]);
            },
