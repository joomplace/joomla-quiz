		...
		if (!$data['q_data']->c_qform) { // radiobuttons
			if ($data['q_data']->c_layout==0) {
				...
				foreach ($qdata as $qone) {
					...
					$jq_tmpl_html .= "<tr><td class='jq_input_pos' ><input id='quest_choice_".$qone->value."' name='quest_choice' value='".$qone->value."' type='radio'  ".($qone->c_right == 1? ' checked="checked" ':"")."><label class='quest_pos $class' for='quest_choice_".$qone->value."'>".stripslashes($qone->text)."</label></td>" . "\n";
					$jq_tmpl_html .= "</tr>" . "\n";
				}			
				$jq_tmpl_html .= "</table>" . "\n";
				...
				foreach ($qdata as $qone) {
					...
					$jq_tmpl_html .= "<input id='quest_choice_".$qone->value."' name='quest_choice' value='".$qone->value."' type='radio'  ".($qone->c_right == 1? ' checked="checked" ':"")."><label for='quest_choice_".$qone->value."'>".stripslashes($qone->text)."</label>" . "\n";				
			...
			$jq_tmpl_html .= '<select name="quest_choice" class="inputbox chzn-select">'. "\n";
			$qoptions = '';
