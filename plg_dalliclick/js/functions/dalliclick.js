...
function setDalliclickImageSize(dalliclickImgRatio) {
...
function startDalliclick(cur_quest_id, stu_quiz_id, button=false) {
	...
	if(jq_jQuery(parent.response).find('sq_delayed').text()) {
		sq_delayed = jq_jQuery(parent.response).find('sq_delayed').text();
	}

	img_width = jq_jQuery(parent.response).find('img_width').text();
	img_height = jq_jQuery(parent.response).find('img_height').text();

	dc_w = Math.floor(img_width/5);
	dc_h = Math.floor(img_height/5);

	var dalliclickImgRatio = jq_jQuery(parent.response).find('img_ratio').text();

	window.addEventListener('resize', function(e) {
		setDalliclickScreenClass();
		setDalliclickImageSize(dalliclickImgRatio);
	});
