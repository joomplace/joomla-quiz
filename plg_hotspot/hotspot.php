...
//		$koef = round($w/$h);
        ...
        if(0){ //$isMobile
            /*
                    ...
                    jq_jQuery('#foo').attr('style', 'width: 100% !important;text-align:center;');
                    ...
                    var rect = paper.rect(0, 0, '100%', '100%').attr({fill:'none'});
		...
		*/
		...
		var paper = Raphael('foo', $imagesizes[0], $imagesizes[1]);
		var img = paper.image('{$live_site}images/joomlaquiz/images/{$data['q_data']->c_image}', 0, 0, '100%', '100%');
		var rect = paper.rect(0, 0, '100%', '100%').attr({fill:'none'});
		...
		img.click(function(event){
			...
			var qqq1 = event.pageY || event.clientY;
			/*var qqq1 = event.pageY || (event.clientY + (document.documentElement.scrollTop || document.body.scrollTop));*/
			/*var qqq2 = event.pageX || (event.clientX + (document.documentElement.scrollLeft || document.body.scrollLeft));*/
			var qqq2 = event.pageX || (event.clientX);
			var base_y = getPosition_y('#foo') + 1;
			var base_x = getPosition_x('#foo') + 1;
			
			cx = Math.round(event.layerX * 100 / jQuery('svg').width());
			cy = Math.round(event.layerY * 100 / jQuery('svg').height());
