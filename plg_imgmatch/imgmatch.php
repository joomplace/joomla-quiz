	...
	function _uploadResizeImg($data)
    {
		...
		if (!empty($user_files)) {
			...
			if (preg_match("/[^0-9a-zA-Z_]/", $filename[0])) {
                echo "<script> alert('".JText::_('COM_JOOMLAQUIZ_FILE_MUST')."');</script>\n";
				die();
			}

			if (JFile::exists($base_Dir.'/'.$userfile_name)) {
				echo "<script> alert('".JText::_('COM_JOOMLAQUIZ_IMAGE').$userfile_name.JText::_('COM_JOOMLAQUIZ_ALREADY_EXISTS')."'); </script>\n";
				die();
			}
			...
			if (!JFile::move($user_files['tmp_name'],$base_Dir.'/'.$user_files['name']) || !JPath::setPermissions($base_Dir.'/'.$user_files['name'])) {
				echo "<script> alert('".JText::_('COM_JOOMLAQUIZ_UPLOAD_OF').$userfile_name.JText::_('COM_JOOMLAQUIZ_FAILED')."'); </script>\n";
				die();
			} else {
