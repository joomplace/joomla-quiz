...
class plgJoomlaquizMemory extends plgJoomlaquizQuestion
{
		...
		if($cols && $cc && $check_pairs){
			...
			for($r=0; $r<=$rows-1; $r++){
				...
				for($c = 0; $c <= $cols-1; $c++){
					...
					$qhtml .= '<img src="'.JURI::root().'images/joomlaquiz/images/memory/'.$data['q_data']->c_img_cover.'" name="img'.(($cc*$r)+$c).'" border="0" style="'.($data['q_data']->c_width ? 'width:'.$data['q_data']->c_width.'px;' : 'width: auto;').' '.($data['q_data']->c_height ? 'height:'.$data['q_data']->c_height.'px;' : '').'">';
					$qhtml .= '</a></td>';
				}
				$qhtml .= '</tr>';
			}
	...
	public function onGetAdminForm(&$data)
	{
		...
		$lists['c_column']['input'] = '<input type="text" size="35" name="c_column" value="'.(isset($row->c_column) ? $row->c_column : '').'">';
