	...
	public static function getReviewContent($review_data, $data){
		...
		if(count($review_data['memory_data'])){
			foreach($review_data['memory_data'] as $mem) {
				$wrong_correct = (in_array($mem->m_id, $review_data['udata'])) ? '<img src="'.JURI::root().'components/com_joomlaquiz/views/templates/tmpl/joomlaquiz_simple/images/tick.png" style="vertical-align:middle;margin-left:10px;"/>' : '<img src="'.JURI::root().'components/com_joomlaquiz/views/templates/tmpl/joomlaquiz_simple/images/tickr.png" style="vertical-align:middle;margin-left:10px;"/>';
