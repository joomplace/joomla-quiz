 ...
 */
class JoomlaquizViewReviewPuzzle
{
    public static function getReviewContent($review_data, $data){
        ...
        for($n = 0; $n < $data['qdata'][0]['difficulty']; $n++){
            for($m = 0; $m < $data['qdata'][0]['difficulty']; $m++){
                $bkg = (!in_array($cc, $review_data)) ? 'background: #eeeeee;filter:alpha(opacity=70);-moz-opacity:.70; opacity:.70;' : 'filter:alpha(opacity=0);-moz-opacity:0; opacity:0;';
