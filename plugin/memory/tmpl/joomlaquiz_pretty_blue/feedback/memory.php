	...
	public static function getFeedbackContent($feedback_data, $data){
		...
		$jq_tmpl_html .= '<table class="jq_feedback_question_content_inner">
							...
							<tr class="jq_feedback_question_content_header">
								...
								<td class="jq_feedback_question_content_col_narrow">'.JText::_('COM_JQ_YOUR_MEMORY_POINTS').'</td>';
		
		$k = 2;
		$sum_score = 0;
		if(count($feedback_data['memory_data'])){
			foreach($feedback_data['memory_data'] as $mem) {
				$wrong_correct = (in_array($mem->m_id, $feedback_data['udata'])) ? '<img src="'.JURI::root().'components/com_joomlaquiz/views/templates/tmpl/joomlaquiz_pretty_blue/images/result_true_green.png" style="vertical-align:middle;margin-left:10px;"/>' : '<img src="'.JURI::root().'components/com_joomlaquiz/views/templates/tmpl/joomlaquiz_pretty_blue/images/result_false.png" style="vertical-align:middle;margin-left:10px;"/>';
				...
				$jq_tmpl_html .= '<tr class="jq_feedback_question_content_container">'.
								...
								'<td class="jq_feedback_question_content_col_narrow">'.$wrong_correct.'</td>'.
								'<td class="jq_feedback_question_content_col_narrow">'.$score.$expl.'</td>'.
								'</tr>';
				$k = 3 - $k;
				$sum_score += $score;
			}
		...
		$jq_tmpl_html .= "<tr><td colspan='3' valign='top'><br /><strong>".JText::_('COM_QUIZ_RES_MES_SCORE')." ".$sum_score."</strong><br /></td>" . "\n";
