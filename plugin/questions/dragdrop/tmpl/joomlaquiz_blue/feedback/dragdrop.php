 ...
 */
class JoomlaquizViewFeedbackDragdrop
{
	public static function getFeedbackContent($feedback_data, $data){
		...
		if(isset($feedback_data['qdata'][0]['past_this'])) {							
			$jq_tmpl_html .= '<tr><td colspan="3" class="review_statistic">'.JText::_('COM_QUIZ_RST_PPAST').' '.$feedback_data['qdata'][0]['past_this'].' '.JText::_('COM_QUIZ_RST_PPAST_TIMES').', '.$feedback_data['qdata'][0]['rht_proc'].'% '.JText::_('COM_QUIZ_RST_ARIGHT').'</td></tr>';
			$jq_tmpl_html .= "<tr><td colspan='3' valign='top'><br /><strong>".JText::_('COM_QUIZ_RES_MES_SCORE')." ".$feedback_data['qdata'][0]['score']."</strong><br /></td>" . "\n";
			$jq_tmpl_html .= "</tr>
							</table>" . "\n";
			$jq_tmpl_html.='</div>' . "\n";
		}
