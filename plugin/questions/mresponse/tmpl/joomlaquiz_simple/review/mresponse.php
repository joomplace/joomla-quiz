...
class JoomlaquizViewReviewMresponse
{
	public static function getReviewContent($review_data, $data){
		...
		foreach ($review_data as $qone) {
			...
			$jq_tmpl_html .= "<tr><td valign='top' class='jq_input_pos'><input id='quest_choice_".$qone->value."' name='quest_choice' value='".$qone->value."' type='checkbox' ".($qone->c_right == 1? ' checked="checked" ': "")." disabled='disabled'></td>
